...
def zber_dat():
    ...
    # --- HTML GENERÁCIA (Soft-Glass Style) ---
    df_last = df.tail(8).copy()
    rows_html = ""
    for _, r in df_last.iterrows():
        t_short = datetime.strptime(r['Čas'], "%Y-%m-%d %H:%M:%S").strftime("%H:%M")
        icon = YR_ICON_MAP.get(r['Symbol'], "bi-cloud-fill")
        
        traffic_cells = "".join([f'<td><div class="traffic-val" style="color:{"#2ecc71" if r[n]>85 else "#f1c40f" if r[n]>60 else "#e74c3c"}">{r[n]}%</div></td>' for n in VJAZDY.keys()])
        park_cells = "".join([f'<td><span class="park-num">{r[f"P_{n}"]}</span></td>' for n in ["Rybníková", "Hospodárska", "Kollárova"]])
    ...
    <head>
        ...
        <style>
            ...
            .nav-bar {{ display: flex; justify-content: center; gap: 30px; margin-bottom: 40px; }}
            .nav-item {{ background: #fff; padding: 10px 25px; border-radius: 100px; font-weight: 600; font-size: 0.85rem; box-shadow: 0 4px 10px rgba(0,0,0,0.03); border: none; }}
            .nav-item.active {{ background: #222; color: #fff; }}
            
            .hero-title {{ font-size: 2.8rem; font-weight: 800; letter-spacing: -1.5px; margin-bottom: 40px; color: #222; }}
            ...
            .table-wrap {{ background: rgba(255,255,255,0.5); border-radius: 30px; padding: 20px; overflow: hidden; }}
    ...
    <body>
        <div class="glass-shell">
            <div class="nav-bar">
                ...
                <button class="nav-item">Nastavenia</button>
            </div>
            ...
            <div class="stats-grid text-center text-md-start">
                ...
                <div class="info-card">
                    ...
                    <div class="progress" style="height: 6px; border-radius: 10px;"><div class="progress-bar" style="width: 70%"></div></div>
                </div>
                <div class="info-card">
                    <span class="card-label">Aktívny monitoring</span>
                    <span class="card-val">24/7</span>
                    <span style="font-size: 0.7rem; font-weight: 700; color: #2ecc71;"><i class="bi bi-circle-fill"></i> SYSTEM ONLINE</span>
                </div>
            </div>
            ...
            <p class="text-center mt-4 small text-muted">Zdroje dát: MET Norway, TomTom, OpenData Trnava</p>
        </div>
    </body>
    </html>
    """
